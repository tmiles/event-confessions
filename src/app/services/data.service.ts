...
export class DataService {
  ...
  updateHomeData(data: { [k: string]: any }): Promise<void> {
    return this.db.doc(`configs/home`).update(data);
  }
  ...
  arrayToMap(data: any[]): { [k: string]: any } {
    let res = {};
    data.forEach((el) => {
      if (!el.id) {
        el.id = this.db.createId();
      }
      res[el.id] = el;
    });
    return res;
