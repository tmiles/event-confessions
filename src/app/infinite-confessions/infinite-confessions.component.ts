...
export class InfiniteConfessionsComponent implements AfterViewInit {
  ...
  constructor(private db: AngularFirestore, private d_s: DataService, private route: ActivatedRoute) {
    this.route.params.subscribe(params => { // todo load the correct event
      this.eventName = params['id'];
      this.db.collection('events').doc(this.eventName).valueChanges().subscribe(res => {
        this.event = res;
      });
  ...
  getBatch(offset) {
    // console.log(offset);
    return this.db.collection('events').doc(this.eventName)
